...
for jj=3:13
   ...
   TWU_province_all=[TWU_province_all,TWU_province];
    
end
Total_WU_5year=[repmat(intersect_value,1,2),TWU_province_all];
% Intersect_data_new=X_Y;
...
for jj=4
    ...
    m=10000;
    cont=0;
    before_P=Total_WU_5year(:,P1);
    after_P=Total_WU_5year(:,P2);
    while cont<m
        ...
        Intersect_data_new=[];
        for kk=1:31
            id0=find(Total_WU_5year(:,2)==current_province(kk));
           ...
           Intersect_data_new=[Intersect_data_new;[X_Y(id0,1),X_Y(id0,2)]];
        end
...
%%
row_title={'provinceId','P_lat','P1_TWU_dir','P_TWU_dir'};
xlswrite('TWU_Gravity_Movement_region_contribute_test.xlsx',[row_title;num2cell([P3_sector_contribute,Sector_TWU_Dir_contribute_all])]);
